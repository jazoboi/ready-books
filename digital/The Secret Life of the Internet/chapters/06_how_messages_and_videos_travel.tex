...
\begin{tcolorbox}[enhanced,colback=SunYellow!12,colframe=SunYellow!70!black,arc=6pt,boxrule=1pt,
  ...
  \begin{itemize}
    \item Data is split into tiny \textbf{packets}, each with a number and a destination address.
    \item \textbf{Routers} direct each packet toward its destination — they can take different routes!
    \item At the other end, all the packets are put back together in the right order.
  \end{itemize}
  Now let's learn the \emph{rules} that make all of this work smoothly\ldots
\end{tcolorbox}
