...
\begin{newwords}
  \begin{description}[labelwidth=3.5cm, leftmargin=3.7cm]
    \item[\term{Processor}]    The brain chip that carries out all instructions.
    \item[\term{Transistor}]   A microscopic on/off switch inside a processor.
    \item[\term{Binary code}]  The language of ones and zeros that computers use.
    \item[\term{Gigahertz}]    A unit of speed: one billion steps per second.
    \item[\term{Instruction}]  A single command the processor must carry out.
  \end{description}
\end{newwords}
